...
<div class="tile">
  <div class="tile-body">
   ...
   $con_table=$db->query("select p.id,p.barcode,p.name,u.name, sum(p.qty),p.unit_price,p.manufacturer,p.description from mr_product p,mr_product_uom u where u.id=p.uom_id and category_id=2 group by p.name");
   ...
   while(list($id,$barcode,$name,$uom,$qty,$price,$mfg,$desc)=$con_table->fetch_row()){
	 ...
	 echo "<tr><td>$id</td><td>$barcode</td><td>$name</td><td>$uom</td><td>$qty</td><td>$price</td><td>$mfg</td><td>$desc</td><td>
			...
			<form action='home.php?page=34' method='post' style='display:inline' onsubmit='return confirm(\"Are you sure?\")'><input type='hidden' name='txtId' value='$id' /><button type='submit' name='btnDelete' class='btn btn-sm btn-danger'><i class='fa fa-trash'></i></button></form></td></tr>";  
   }   
?>
</table>
</div>
